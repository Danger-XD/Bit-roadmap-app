...
const RoadContents = () => {
  ...
  return (
    <>
      <div className="category-filter absolute top-0 right-2">
        ...
        <div className="w-full mt-14 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {responseItems.map((item, index) => (
            <div key={index} className="flex justify-center">
              <Link
                to={`/roadmap/one/${item._id}`}
                className="h-full w-[22rem] sm:w-full font-semibold flex justify-between px-5 py-2 border rounded-2xl hover:text-white hover:bg-orangy"
              >
                <p>{item.title}</p>
                <div className="flex">
                  ...
                  <p className="flex justify-center items-center pl-3">
                    <FaRegCommentAlt />
                    &nbsp;{item.commentCounts}
                  </p>
                </div>
